...
const AddProductModalBody = ({ closeModal }) => {
  ...
  const handleImageChange = (event) => {
    ...
    setFormData((prevFormData) => ({
      ...prevFormData,
      photo: imageFile, // Store the file object
    }));
  };

  const onSubmit = async (submittedData) => {
    // Convert status from true/false to active/inactive
    submittedData.status = submittedData.status ? "active" : "inactive";
    ...
    for (const key in submittedData) {
      if (key === "photo") {
        ...
        if (formData.photo instanceof File) {
          dataToSend.append("photo", formData.photo); // New photo selected
        } else {
          dataToSend.append("photo", data?.data?.product?.photo); // Existing photo
        }
      } else {
        dataToSend.append(key, submittedData[key]); // Append other form fields
      }
    }

    try {
      if (id) {
        await updateSingleProduct(id, dataToSend);
      } else {
        await addProductHandler(dataToSend);
      }
